...
public interface BookingRepository extends JpaRepository<Booking, Long>, JpaSpecificationExecutor<Booking> {
  @Query("SELECT b FROM AdminBooking b WHERE lower(cast(b.customer.name as string)) LIKE lower(concat('%', :q, '%')) "
  ...
  @Query("SELECT COUNT(b) FROM AdminBooking b WHERE b.status = 'PENDING'")
  long countPending();
}
