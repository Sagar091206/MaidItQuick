# US-01 — Shared UI States (Skeleton, Empty, Error, Offline, Success, Retry)

> Master prompt: `00_MASTER_SYSTEM_PROMPT.md`. Read it before starting.
> This story is the foundation for every screen in the remaining stories. It creates **one new file** and modifies **no existing file**.

## 1. Objective

Create a reusable, dependency-free set of UI-state widgets in `mobile/lib/shared/widgets/` so every customer screen renders consistent loading (pulsing skeletons), empty, error (with retry), offline, and success states. The existing `_EmptyState`, `_DashboardError`, `_StatusPill` widgets duplicated inside screens will be replaced by these shared components in later stories.

## 2. Scope

- **In:** new shared widgets, a central `StatusPill`, a `SectionHeader`.
- **Out:** no screen changes, no backend changes, no new packages (no `shimmer`, no `lottie`).

## 3. Tech stack details

- Flutter / Dart (existing stack). `flutter_lints` clean.
- Pure `StatelessWidget` + one `StatefulWidget` (`_Pulse`) with an `AnimationController`.
- Uses `core/brand_theme.dart` helpers (`context.brandMuted`, `context.brandCard`, `context.scheme`).

## 4. New file

`mobile/lib/shared/widgets/app_states.dart`

## 5. Implementation — Dart (full file)
...
## 6. UI states checklist (this file satisfies)

- Loading → `SkeletonListView`, `SkeletonPulse`, `SkeletonBox`, `SkeletonListTile`, `SkeletonCard`.
- Empty → `EmptyStateView`.
- Error + Retry → `ErrorStateView` (and `OfflineBanner.onRetry`).
- Offline → `OfflineBanner`.
- Success → `SuccessCheck`.
- Shared chrome → `StatusPill`, `SectionHeader`, `bookingStatusLabel`, `formatPaise`, `formatDateTime`.

## 7. Tests

`mobile/test/shared/widgets/app_states_test.dart` (new):
...
flutter analyze
flutter test test/shared/widgets/app_states_test.dart
```

## 9. Acceptance criteria

- [ ] `app_states.dart` compiles cleanly; no new packages added.
- [ ] Skeleton widgets pulse; no shimmer package.
- [ ] All five UI states available as shared widgets.
- [ ] `StatusPill`, `SectionHeader`, label/format helpers are reusable and used by later stories.
